...
function Boards() {
  //So luong ban ghi boards hien thi toi da tren 1 page
  const [boards, setBoards] = useState(null);
  //Tong toan bo so luong ban ghi boards tu Database
  const [totalBoards, setTotalBoards] = useState(null);

  //xu ly phan trang url với MUI: https://mui.com/material-ui/react-pagination/#router-integration
  const location = useLocation();
  /**
   * Parse chuoi string search trong location ve doi tuong URLSearchParams
   * https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams/URLSearchParams
   */
  const query = new URLSearchParams(location.search);
  /**
   * Lay gia tri page tu query, default la 1
   * ParseInt
   */
      ...
      <Box sx={{ paddingX: 2, my: 4 }}>
        <Grid container spacing={2}>
          ...
          <Grid xs={12} sm={9}>
            ...
            {/*Truong hop: goi API khong ton tai board nao het*/}
            {boards?.length === 0 && (
            ...
            {/*Truong hop: goi API ton tai board => Danh sach boards */}
            {boards?.length > 0 && (
              <Grid container spacing={2}>
                {boards.map((b) => (
            ...
            {/*Truong hop: Goi API co totalBoards => Render khu vuc phan trang*/}
            {totalBoards > 0 && (
              <Box
                sx={{
                  my: 3,
